...
const ALARM_BK_ARRAY_SCHEMA = Joi.array().items(ALARM_BK_COLLECTION_SCHEMA);
...
const createNew = async (data) => {
  try {
    ...
    const create = await GET_DB().collection(ALARM_BK_COLLECTION_NAME).insertMany(validData)
...
const getAlarms = async (startDate, endDate) => {
  try {
    ...
    const data = await GET_DB().collection(ALARM_BK_COLLECTION_NAME).find({
      date: {
        $gte: startDateInt,
        $lte: endDateInt
      }
...
//
export const station1AlarmModel = {
  ALARM_BK_COLLECTION_NAME,
  ALARM_BK_COLLECTION_SCHEMA,
  createNew,
  getAlarms
}
