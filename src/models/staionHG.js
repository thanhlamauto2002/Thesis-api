...
const getDataChart = async (option) => {
  try {
    ...
    switch (option) {
      case 'today':
        data1 = await GET_DB().collection(HG_COLLECTION_NAME).find({ 'createdAt': { $gte: startOfToday } }).toArray()

        break
      case 'l7day':
        data1 = await GET_DB().collection(HG_COLLECTION_NAME).find({ 'createdAt': { $gte: sevenDaysAgoUnix } }).toArray()
        break
      case 'l30day':
        data1 = await GET_DB().collection(HG_COLLECTION_NAME).find({ 'createdAt': { $gte: thirtyDaysAgoUnix } }).toArray()
        break
      default:
